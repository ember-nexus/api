...
- Error responses: cut-off request bodies (Caddy body timeout, aborted connection) answer `408` (new error page
  `/error/408/request-timeout`, also `/error/503/service-unavailable`, both with example tests); `docs/server/` holds
  the responses generated by Caddy itself (they mirror the headers of `public/index.php`, keep both in sync).
  Open: `log_errors` is `0` in `php.prod.ini`, so PHP failures before Symfony boots are not logged at all, and client
  errors log only type and title, not the detail; a request id header would help to correlate `see log` errors.
- Every problem json response (also the ones created by Caddy) identifies its request as `instance`
  (`urn:uuid:<id>`); the id is also the `requestId` of the application log, the `request_id` of the Caddy access log and
  the `uuid` of PHP error lines, use it to look up the log lines. It is not a response header. The example files hold
  a placeholder value, which the comparison ignores. Caddy-created problem responses use a relative `type`
