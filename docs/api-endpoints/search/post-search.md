...
- [Cypher Path](/search/step/cypher-path-subset): Ideal to find collections of elements based on the relation
  between them. Uses the [Cypher Path Subset](/concepts/grammars?id=cypher-path-subset) query language, see also
  [it's railroad diagram](https://ember-nexus.github.io/api/antlr/cypher-path-subset.html).
