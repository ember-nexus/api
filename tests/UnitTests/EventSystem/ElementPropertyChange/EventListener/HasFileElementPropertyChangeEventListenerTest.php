...
use App\EventSystem\ElementPropertyChange\EventListener\HasFileElementPropertyChangeEventListener;
...
#[Small]
#[CoversClass(HasFileElementPropertyChangeEventListener::class)]
class HasFileElementPropertyChangeEventListenerTest extends TestCase
{
    use ProphecyTrait;

    public function testElementsWithoutHasFilePropertyAreIgnored(): void
    {
        self::expectNotToPerformAssertions();
        $event = new ElementPropertyChangeEvent('Test', null, ['name' => 'Test']);
        $eventListener = new HasFileElementPropertyChangeEventListener(
            $this->prophesize(Client400ForbiddenPropertyExceptionFactory::class)->reveal()
        );
        $eventListener->onElementPropertyChangeEvent($event);
    }

    public function testElementWithHasFilePropertyTriggersException(): void
    {
        $event = new ElementPropertyChangeEvent('Test', null, ['hasFile' => true]);
