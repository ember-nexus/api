 ...
 *         min?: int, // Minimum number of elements which are always returned, if they exist. // Default: 5
 *         default?: int, // Default number of elements which are returned if they exist. // Default: 25
 *         max?: int, // Maximum number of elements which are returned in a single response. Should not be way more than 100, as performance problems may arise. // Default: 100
 *     },
 *     register?: array{ // Handles the /register endpoint.
 *         enabled?: bool, // If true, the /register endpoint is active and anonymous users can create accounts. // Default: true
 *         uniqueIdentifier?: scalar|null, // The property name of the identifier. Identifier must be unique across the API, usually the email. // Default: "email"
 *         uniqueIdentifierRegex?: scalar|null, // Either false or a regex for checking the identifier content. // Default: false
 *     },
 *     expression?: array{ // Configures settings related to the execution of expressions
 *         enabled?: bool, // If true, expressions are executed. // Default: true
 *         warningLength?: int, // Minimum length for executable expressions to trigger a warning in the logs. // Default: 512
 *         maxLength?: int, // Maximum length for executable expressions. Exceeding the limit will throw an exception. // Default: 10240
 *     },
 *     instanceConfiguration?: array{ // Configures the /instance-configuration endpoint.
 *         enabled?: bool, // If true, enables the endpoint. If false, 403 error messages are returned. // Default: true
 *         showVersion?: bool, // If false, the version number is omitted. // Default: true
 *     },
 *     token?: array{ // Configures token settings.
 *         minLifetimeInSeconds?: int, // Minimum lifetime of created tokens. // Default: 1800
 *         defaultLifetimeInSeconds?: int, // Default lifetime of created tokens. // Default: 10800
 *         maxLifetimeInSeconds?: scalar|null, // Maximum lifetime of created tokens. Can be set to false to disable maximum limit. // Default: 34214400
 *         tokenDeleteExpiredTokensAutomaticallyInSeconds?: scalar|null, // Expired tokens will be deleted after defined time. Can be set to false to disable auto delete feature. // Default: 1209600
 *     },
 *     cache?: array{ // Configures the cache.
 *         etagSeed?: scalar|null, // Seed used to generate Etag values. Changing it invalidates all data cached by clients. // Default: ""
 *         etagUpperLimitInCollectionEndpoints?: scalar|null, // Maximum number of items in a collection for which to generate Etags. Bigger collection do not contain Etags due to performance problems. // Default: 100
 *     },
 *     featureFlag?: array{ // Configures feature flags.
 *         280_OldUniqueUserIdentifierDisabled?: bool, // If true, the original behavior of some user endpoints in versions before 0.1.6 will no longer be supported. Old behavior will be dropped with version 0.2.0. See GitHub issue #280. // Default: false
 *     },
 * }
