...
class IfNoneMatchControllerEventListener
{
    ...
    public function onKernelController(ControllerEvent $event): void
    {
        ...
        $etags = [];
        $matchesAnyEtag = false;
        foreach ($rawEtags as $rawEtag) {
            $rawEtag = trim($rawEtag);
            if ('*' === $rawEtag) {
                // matches every current representation; an endpoint without one has no current ETag and never gets here
                $matchesAnyEtag = true;
                continue;
            }
        ...
        if ($matchesAnyEtag || in_array($currentRequestEtag->getEtag(), $etags)) {
